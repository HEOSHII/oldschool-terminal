...
const display = reactive({ content: [content.init], isTerminalBusy: false, isTerminal: false });
...
<template>
  ...
  <main v-if="display.isTerminal"
    ...
    <Input v-if="!display.busy" :callCommand="command => runCommand(command, display)" />
    <FakeInput v-else />
  </main>
</template>
